...
- `moderators` table: moderation only (`is_moderator()` = moderator or admin) — sees pending profiles, approves (update), writes/reads `moderation_actions`. Admins promote/demote moderators from the panel (`search_users` RPC finds accounts by email).
...
- `supabase/migrations/` — SQL schema, RLS, triggers (source of truth for DB). Apply in numeric order 0001 → 0017. Convention: every table carries `created_at` + `updated_at` (updated_at auto-stamped by the shared `set_updated_at()` BEFORE UPDATE trigger, migration 0016) — new tables must add both + the trigger. `supabase/seed.sql` (service-role, run once) bootstraps the launch feed with migrated/claimable people.
