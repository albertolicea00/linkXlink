...
- PWA: vite-plugin-pwa (`registerType: 'prompt'` + `injectRegister: false` — updates surface as a reload toast via `UpdatePrompt.tsx`/`useRegisterSW`, never swap in silently mid-session since RPC signatures change across deploys; runtime caching via Workbox: Supabase REST StaleWhileRevalidate, Storage images CacheFirst — handles photo/network-response caching, unchanged). Custom install button (`useInstallPrompt.ts`, shown in `NavBar` only when installable) since browsers increasingly suppress the native prompt; manifest `shortcuts` for /app and /account (long-press the home-screen icon). App-level offline data lives in IndexedDB (`src/lib/idb.ts`, no dependency): the profiles feed (`useProfiles`) falls back to the last successfully fetched list when a request fails, and `profile_events` (view/whatsapp_click) that fail to send queue there and flush automatically on the `online` event (`lib/metrics.ts` `flushPendingEvents`, wired in `main.tsx`) — a pragmatic retry, not the true Background Sync API (would need `injectManifest` instead of `generateSW`). Small synchronous device flags (swap/click counters, dev flags, terms-accepted, seen-profiles rotation) stay in localStorage on purpose — they're tiny and don't need IndexedDB's async structured-storage trade-offs. Neither storage is "more secure" than the other; both are equally same-origin/XSS-readable.
...
- Config is split in THREE files under `src/config/`, by what the value *is*, not by which page reads it. Change behavior there, never with magic numbers:
  - `app-config.json` (imported as `appConfig`) — behavior knobs: languages, `auth_providers`, app gate toggles, preview count, report/swap/click limits, share gate size, WhatsApp prefill + deep link kill switches, swipe threshold, preload count, rotation, deck counter/stats/undo visibility, `swipe_hint_swipes` / `swipe_hint_reset_hours`, tracking flags, `report_threshold`, `moderation_approve_quorum`, `moderation_deny_quorum`, `moderation_deny_reasons`, photo limits, fallback image variants, `show_age`, `gender_options`, `interest_options`, `max_interests`. Quorum thresholds are ALSO stored authoritatively in `app.settings` (`approve_quorum` / `deny_quorum`); config mirrors them for UI, the `moderate_profile` RPC enforces the DB copy.
  - `app-links.json` (`appLinks`) — every outward-facing URL and the project's identity: `site_url`, `site_alt_urls`, `app_source_url`, `community_{telegram,whatsapp,twitter,facebook,instagram,tiktok}_url` (empty = that CTA hidden; all socials empty = the whole "follow us" card hidden), `community_banner_snooze_days`, `author_handle`, `author_website_url`, `support_coffee_url`. **The identity/donation keys are legally protected — see CONTRIBUTING.md; do not edit them in a fork or PR.**
  - `dev-config.json` (`devConfig`) — admin/dev-only surface: `show_app_settings_to_admins`, `show_dev_settings_to_admins`, `seed_profiles_visible_before_claim`, `dev_flags`.
...
- Community/support: single Telegram channel (`community_telegram_url` in app-links) doubles as bug reports, feature requests and customer service for now. Shown as a banner on `/account` and as step 6 on the landing.
