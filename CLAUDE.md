...
- `supabase/migrations/` — SQL schema, RLS, triggers (source of truth for DB). Apply in numeric order 0001 → 0019. Convention: every table carries `created_at` + `updated_at` (updated_at auto-stamped by the shared `set_updated_at()` BEFORE UPDATE trigger, migration 0016) — new tables must add both + the trigger. `supabase/seed.sql` (service-role, run once) bootstraps the launch feed with migrated/claimable people.
